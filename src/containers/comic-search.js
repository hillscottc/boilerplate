...
import { getMultiSelected, resetMultiSelect, getQueryString, compareValues, sortByLabel } from '../utils'
...
class ComicSearch extends Component {
  ...
  canReset  = () => {
    const { optSubsSelected, optDpwSelected, optRRSelected, refNum, countries, publishers,
      properties, catProps, franchises, formats, title, shelfStartDate, shelfEndDate,
  ...
  getFormData = () => {
    const { optMagName, optComicType, offset} = this.state;
  ...
  render() {
    ...
    const { searchComicClick, selectDpw, selectRR, titleChoose,
      ...
      selectSubscribed, clickSubscribe, selectMagName, selectFormat, selectComicType,
    ...
    const { loaded, comics, title, refNum,
      ...
      showSubscribeModal, modalTitleId, subscriptions, formats,
      optMagName, optSubsSelected, optComicType, dmcpName, selectDmcByVal, sources,
